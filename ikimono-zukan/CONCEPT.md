...
### フェーズ2：MVP —— **実装ずみ**（[src/](src/)）

| | 実装 | 備考 |
|---|---|---|
| 1 | `Setup.gs` | 4シートを自動生成。メニューから1回実行すれば動く |
| 2 | `GeminiApi.gs` | `responseJsonSchema` ＋ `googleSearch` の併用。モデル名は差し替え可能 |
| 3 | `Kana.gs` / `Repo.gs` | 正規化・ゆるいキー・別名索引・`LockService` による直列化 |
| 4 | `Safety.gs` | 要注意リストによる上書き。**初期データ40件を投入ずみ** |
| 5 | `Images.gs` | Wikipedia → Commons → 生成 のフォールバック、Drive保存 |
| 6 | `Api.gs` | 検索フローの組み立て。画面に渡す時点で「どのカードを出さないか」まで解決する |
| 7 | `index.html` / `css.html` / `js.html` | 5画面。Chromebook 横画面（1366×650）最適化ずみ |
| 8 | `tests/harness.test.js` | GASを模したサンドボックスで70件。ネットワーク不要 |

**フェーズ3の一部を前倒しで実装した項目**
- 画面3（みんなの図鑑一覧・カテゴリ絞り込み）
- ふりがな自動付与（`｜漢字《かんじ》` 記法 → `<ruby>`）
- 保護者向けの管理（`status` を `hidden` にすると画面から消える／シートを直接書き換えれば即反映）

### フェーズ3（残り）
9. 読み上げ機能（`SpeechSynthesis`。低学年が一人で使えるようにする）
10. 一覧の並べ替え（新着順・よく見られている順）
11. 「写真なし」の生き物をまとめて確認できる保護者向けビュー
